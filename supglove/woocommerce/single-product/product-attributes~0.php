...
$productspecs = [
  ...
  __( 'Material', 'woocommerce' ) =>
      ($material = implode(", ", array_values(wc_get_product_terms($product->id, 'pa_chem_dispos_gloves_material', array('fields' => 'names'))))) ? $material : (($material = implode(", ", array_values(wc_get_product_terms($product->id, 'pa_material', array('fields' => 'names')))))
          ? $material
          : __('N/A', 'woocommerce')),
	__( 'Thickness', 'woocommerce' ) => ( $thickness = implode(", ", array_values( wc_get_product_terms( $product->id, 'pa_chem_dispos_gloves_thickss', array( 'fields' => 'names' ) ) ) ) ) ? $thickness : __( 'N/A', 'woocommerce' ),
 	...
 	__( 'Instructions', 'woocommerce' ) => ( $instructions = implode(", ", array_values( wc_get_product_terms( $product->id, 'pa_laundering_instructions', array( 'fields' => 'names' ) ) ) ) ) ? $instructions : __( 'N/A', 'woocommerce' ),
];
...
$productspecs += [
	//__( 'Material', 'woocommerce' ) => ( $material = implode(", ", array_values( wc_get_product_terms( $product->id, 'pa_material', array( 'fields' => 'names' ) ) ) ) ) ? $material : __( 'N/A', 'woocommerce' ),
 	__( 'Gauge', 'woocommerce' )  => ( $materialgauge = implode(", ", array_values( wc_get_product_terms( $product->id, 'pa_material_gauge', array( 'fields' => 'names' ) ) ) ) ) ? $materialgauge : __( 'N/A', 'woocommerce' ),
...
foreach ( $productfeatures as $features_name => $features_value ) {
  if ( $features_value !== 'N/A' &&
      strtolower($features_name) != "made-in-canada.png" && strtolower($features_name) != "none" &&
      strtolower($features_name) != "water resistent (material)" && strtolower($features_name) != "oil/water repelleant (treatment)"
  ) {
