...
function supro_author_box() {
	...
	if ( 'full-content' == $layout ) {
		$col = 'col-md-4 col-md-offset-4 col-xs-12 col-sm-12';
	}
				...
				<div class="post-author-box clearfix">
					...
					<div class="post-author-info">
						...
						<?php
							...
							echo sprintf(
								'<span class="authorfoot">' .
								'<a href="%s" target="_self" class="buttonogs btn_small btn_white icon_pos_after  "><span>Read More By %s</span> <i class="icon-arrow-right"></i> </a></span>',
...
function supro_entry_meta_cats() {
	$fields = (array) supro_get_option( 'blog_entry_meta' );
	...
	echo '<div class="entry-metas cats">';
...
function supro_single_entry_meta() {
	$fields =  array_reverse ( (array) supro_get_option( 'post_entry_meta' ));
...
function supro_shop_cta() {
    $count_posts = wp_count_posts();

    if(! is_single()) {
        echo '<div class="blog_count"><span>'. $count_posts->publish .' ' . __('Items') . '</span> | <a href="'. get_permalink( woocommerce_get_page_id( 'shop' ) ) .'">' . __('View All') . '</a></div>';
    }

    echo '<div class="allgloves"><a  href="'. get_permalink( get_option( 'page_for_posts' ) ).'" class="buttonogs btn_small btn_theme_color">'.esc_html__( 'View All Blog Articles', 'supro' ).'</a></div>';
...
if ( ! function_exists( 'supro_recently_viewed_products' ) ) :
	function supro_recently_viewed_products( $atts ) {
		...
		$viewed_products = apply_filters( 'supro_get_recently_viewed', $viewed_products );
...
if ( ! function_exists( 'supro_taxs_list' ) ) :
	function supro_taxs_list( $args ) {
		...
		if ( $output ) {
			...
			echo '<div id="supro-taxs-list" class="supro-taxs-list">' . $output . '
			<form id="search_blog" action="/" method="get">
                <input type="text" name="s" placeholder="Search the Blog" id="search" value="'. the_search_query() .'" />
                <input type="hidden" value="post" name="post" id="post_type" />
                <button type="submit"><i class="t-icon icon-magnifier"></i></button>
            </form>
			<div class="navrightsub"><a href="#" target="_self" class="topsubscribe buttonogs btn_small btn_white2 icon_pos_after  "><span>Subscribe</span> <i class="icon-arrow-right"></i> </a></div></div>';
...
/**
 * Get page header layout FOR SINGLE BLOG POST
 *
 * @return array
 */

if ( ! function_exists( 'supro_get_page_header_single_post' ) ) :
	function supro_get_page_header_single_post() {
      ...
      if ( !is_single() ) {
		...
		$page_header = array(
		  ...
		  //		return false;
			...
			$bg_image           = get_post_meta( get_the_ID(), 'page_header_bg_blog', true );  		
			$position_desktop   = get_post_meta( get_the_ID(), 'page_header_background_position_blog', true );
			$size_desktop       = get_post_meta( get_the_ID(), 'page_header_background_size_blog', true );
			$repeat_desktop     = get_post_meta( get_the_ID(), 'page_header_background_repeat_blog', true );
			$attachment_desktop = get_post_meta( get_the_ID(), 'page_header_background_attachment_blog', true );
...
if ( ! function_exists( 'supro_get_page_base_url' ) ) :
	function supro_get_page_base_url() {
		if ( defined( 'SHOP_IS_ON_FRONT' ) ) {
			...
			$link = get_term_link( get_query_var( 'term' ), 'product_cat' );
...
if ( ! function_exists( 'supro_get_product_zoom' ) ) :
	function supro_get_product_zoom() {
		...
		//return sprintf(			'<a href="#" class="gallery-item-icon item-icon"><span>%s</span></a>', 			esc_html__( 'Click to enlarge', 'supro' ) );
		
		return '<a href="#" class="gallery-item-icon item-icon"></a>';
...
/**
 * Get product Kevlar icon
 */
if(! function_exists('supro_get_product_kevlar_icon')) {
    function supro_get_product_kevlar_icon() {
        global $product;

        $kevlar_data = get_post_meta($product->get_id(), 'kevlar_icon');
        if(!empty($kevlar_data[0]) && $kevlar_data[0] != 'None') {
            return '<div class="kevlar_icon"><img src="'. get_template_directory_uri() .'/img/product_attribute_icons/kevlar.svg" /></div>';
        }
    }
}