...
class Supro_Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $item_mega_background ) {
			...
			if ( isset( $item_mega_background['image'] ) && $item_mega_background['image'] ) {
			...
			if ( isset( $item_mega_background['repeat'] ) && $item_mega_background['repeat'] ) {
				$inline .= ' ; background-repeat:' . esc_attr( $item_mega_background['repeat'] );
			}

			if ( isset( $item_mega_background['size'] ) && $item_mega_background['size'] ) {
				$inline .= '; background-size:' . esc_attr( $item_mega_background['size'] );

			}
			if ( isset( $item_mega_background['attachment'] ) && $item_mega_background['attachment'] ) {
				$inline .= '; background-attachment:' . esc_attr( $item_mega_background['attachment'] );
			}

			if ( isset( $item_mega_background['color'] ) && $item_mega_background['color'] ) {
